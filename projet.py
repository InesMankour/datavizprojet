...
def loaddata(url):
    ...
    df=pd.read_csv(url,usecols=req_col,parse_dates=['date_mutation'],dtype={("nature_mutation ","nom_commune","nature_culture",'adresse_nom_voie'):"category",("valeur_fonciere","surface_relle_bati","nombre_pieces_principales","surface_terrain","longitude","latitude") : "float32",'code_departement': "string"} )
    df['date_mutation']=pd.to_datetime(df["date_mutation"])
    df.dropna(axis=0, inplace=True)
    df['month']=df['date_mutation'].map(get_month)
    return(df)

df2020=loaddata('https://jtellier.fr/DataViz/full_2020.csv')
df2019=loaddata('https://jtellier.fr/DataViz/full_2019.csv')
df2018=loaddata('https://jtellier.fr/DataViz/full_2018.csv')
df2017=loaddata('https://jtellier.fr/DataViz/full_2017.csv')
df2016=loaddata('https://jtellier.fr/DataViz/full_2016.csv')
...
def mapslider(df):
        ...
        st.subheader(f'Map de toutes les ventes durant le mois  {month_to_filter} de 2020')
...
def main(app_mode):
    ...
    if app_mode== "Accueil":
        ...
        st.write(" Voici une carte legendée en couleur par la valeur foncière moyenne de chaque département")
        ...
        st.write("Valeur foncière en fonction des mois de l'année")
        ...
        annee=st.selectbox("",[2016,2017,2018,2019,2020])
    ...
    if app_mode== "Recherche/ Vente Appart":
        ...
        choixdep = st.selectbox("choisissez votre departement",departements)
        df2=df2020[df2020["code_departement"] == choixdep]
        communes=df2['nom_commune'].unique()
        communes.sort()
        choixcom=st.selectbox("choisissez votre commune",communes)
        df3=df2[df2["nom_commune"] == choixcom]
        rues=df3['adresse_nom_voie'].unique()
        rues.sort()
        choixrue=st.selectbox("choisissez votre rue",rues)
